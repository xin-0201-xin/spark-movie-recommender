...
integer_rdd = sc.textFile("/FileStore/tables/integer.txt")

# Convert each line to integer
numbers = integer_rdd.map(lambda x: int(x.strip()))

# Count even and odd numbers
even_count = numbers.filter(lambda x: x % 2 == 0).count()
odd_count = numbers.filter(lambda x: x % 2 != 0).count()
...
#partA Q2
#load and process the salary data
Salary = spark.sparkContext.textFile("/FileStore/tables/salary.txt")
arraySalary = Salary.map(lambda x: x.split(" "))
Number = arraySalary.map(lambda x: (x[0], int(x[1])))

# Convert RDD to DataFrame with columns "department" and "salary"
columns = ["department", "salary"]
salaryDF = Number.toDF(columns)
salaryDF.cache()


import pyspark.sql.functions as F

# Calculate basic statistics per department: mean, standard deviation, minimum and maximum
stats_df = salaryDF.groupBy("department").agg(
...
# Calculate the median per department using approxQuantile
departments = [row.department for row in salaryDF.select("department").distinct().collect()]
median_stats = []
for dept in departments:
    median_val = salaryDF.filter(salaryDF.department == dept)\
                         .approxQuantile("salary", [0.5], 0.01)[0]
    median_stats.append((dept, median_val))

medianDF = spark.createDataFrame(median_stats, ["department", "median_salary"])


# Optionally, join the statistics into one DataFrame for easier review
stats_joined = stats_df.join(medianDF, "department")
display(stats_joined)

import matplotlib.pyplot as plt
import pandas as pd

# Convert Spark DataFrame to Pandas DataFrame for plotting
pdf = salaryDF.toPandas()

# Create a box plot for salary distribution by department
plt.figure(figsize=(10, 6))
pdf.boxplot(column="salary", by="department")
...
# Create a histogram for overall salary distribution
plt.figure(figsize=(10, 6))
plt.hist(pdf["salary"], bins=20, edgecolor="black")
plt.title("Overall Salary Histogram")
plt.xlabel("Salary")
...
# MAGIC %md
# MAGIC From above analysis, we can see that sales department has the highest mean salary while marketing department has the lowest mean salary. And the sales department can reach the much higher salary than others sometimes.  
...
shakespeare_rdd = sc.textFile("/FileStore/tables/shakespeare_1.txt")

# Define the target words (all lower-case for uniformity)
...
# MapReduce: Count only the target words
word_counts = (shakespeare_rdd
               .flatMap(clean_and_split)
...
# Define a function to clean each line (remove punctuation and convert to lower-case)
def clean_and_split(line):
    # Remove punctuation using a regular expression and convert to lower-case
    line_clean = re.sub(r'[^\w\s]', '', line).lower()
    return line_clean.split()

# Create an RDD of all words
all_words = shakespeareRDD.flatMap(clean_and_split)

# Count the occurrences of each word
word_counts = all_words.map(lambda word: (word, 1)).reduceByKey(lambda a, b: a + b)

# Get top 10 words by descending frequency
top_10 = word_counts.takeOrdered(10, key=lambda x: -x[1])
print("Top 10 words:")
for word, count in top_10:
    print(f"{word}: {count}")

# Get bottom 10 words by ascending frequency (words with the smallest counts)
bottom_10 = word_counts.takeOrdered(10, key=lambda x: x[1])
print("\nBottom 10 words:")
